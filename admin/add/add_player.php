	...
	if (isset($_POST)) {
		...
		if (mysqli_num_rows($result_find_add_player) == 0) {
			...
			if ($result_add_player) {
				...
				echo "<a href='?page=player&sub_page=add&resuit=Yes$posttoget'>Yes</a><br><a href='?page=player&sub_page=add&resuit=No'>No</a>";
				unset($_POST);
				switch ($_GET['resuit']) {
					case 'Yes':
						...
						$sql_update_player = "UPDATE `players` SET `player_title`='$player_title',`player_name`='$player_name',`player_mid_name`='$player_mid_name',`player_sirname`='$player_sirname',`player_class`='$player_class',`player_room`='$player_room',`player_gender`='$player_gender',`player_color_id`='$player_color_id' WHERE `player_id`='$player_id'";
